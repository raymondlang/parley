...
const WorkspacePrimaryView = () => {
  ...
  const messageableType = messageableId.includes("c")
    ? "channel"
    : "directMessage";
  ...
  const messageName = useSelector((state) => {
    if (messageableType === "channel") {
      ...
      const userNameArr =
        state.directMessages[messageableId.slice(2, 100) * 1].name;
      let name;
  ...
  const messageMembersArr = useSelector((state) => {
    if (messageableType === "channel") {
      ...
      return state.directMessages[messageableId.slice(2, 100) * 1]
        .workspaceUsers;
    }
  });
